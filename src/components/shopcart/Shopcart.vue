...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      let totalPri = 0;
      this.selectFoods.forEach(item => {
        totalPri += item.min_price * item.count;
      });
      return totalPri;
    },
    payStr() {
    ...
    listShow() {
      ...
      if (!this.totalCount) {
        ...
        let show = !this.fold;
        // 引入的滚动模块better-scroll 当点击购物图标的时候 且购物车有值的时候开始实例化 better-scroll
        if (show) {
          //如果show为true,也就是展开购物车清单的时候 实例化better-scroll
          // nextTick()方法是等DOM加载完毕再执行
          this.$nextTick(() => {
            if (!this.shopScroll) {
              //第一次初始化判断 shopScroll是否存在 不存在 就是实例化一个BScroll 指定滚动的位置
  ...
  methods: {
    ...
    toggleList() {
      if (!this.totalCount) {
        //如果这个购物车里面没有数据 那就直接return掉
        return;
      } else {
        this.fold = !this.fold;
      }
    },
    //清空购物车:遍历所有数组中的count为0；
    clearAll() {
